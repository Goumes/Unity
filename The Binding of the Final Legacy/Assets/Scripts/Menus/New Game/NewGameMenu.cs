...
public class NewGameMenu : MonoBehaviour
{
    ...
    /// <summary>
    /// Method that clears all the slices in the menu and selects the first attack button
    /// </summary>
    ...
    void Update ()
    {
        ...
        if (inputName.text.Length < 2) //La longitud es 1 cuando no hay nada en este componente por alguna extraña razón...
        {
    ...
    /// <summary>
    /// Method that checks which button you have selected, and calls its corresponding method depending on which direction the player pressed
    /// </summary>
    ...
    public void startButton()
    {
        ...
        gameDataManager.createdPlayer = new PlayerClass(inputName.text, healthValue * 50.0f, 100.0f, defenseValue * 1.2f, attackValue + 1.2f, wealthValue * 100.0f /*99999.0f*/, new List<Ability>(), new List<GenericItem>(), null, null, 1);//Aqui hay que multiplicar el valor del atributo por lo que sea
        gameDataManager.selectedSave = randomName;
        gameDataManager.lvl = 1;
        SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex + 1);
    }

    /// <summary>
    /// Coroutine that makes the point counter blink every time the player tries to add more points than it has left.
    /// </summary>
    /// <returns></returns>
